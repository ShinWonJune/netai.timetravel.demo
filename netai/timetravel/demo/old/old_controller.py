...
class TimeController:
    ...
    def __init__(self):
        ...
        if self._sensor_data:
            ...
            # Z 형식 타임스탬프 파싱 처리
            try:
                if ".000Z" in first_timestamp:
                    self._start_time = dt.strptime(first_timestamp, "%Y-%m-%dT%H:%M:%S.%fZ")
                    self._end_time = dt.strptime(last_timestamp, "%Y-%m-%dT%H:%M:%S.%fZ")
                elif "Z" in first_timestamp:
                    self._start_time = dt.strptime(first_timestamp, "%Y-%m-%dT%H:%M:%SZ")
                    self._end_time = dt.strptime(last_timestamp, "%Y-%m-%dT%H:%M:%SZ")
                else:
                    self._start_time = dt.strptime(first_timestamp, "%Y-%m-%dT%H:%M:%S")
                    self._end_time = dt.strptime(last_timestamp, "%Y-%m-%dT%H:%M:%S")
    ...
    def _datetime_to_timecode_value(self, dt_obj):
        ...
        try:
            base_dt = dt.strptime(base_time_str, "%Y-%m-%dT%H:%M:%SZ")
